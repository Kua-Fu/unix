...
fork()
{
	...
	p1 = u.u_procp; // 将p1指向执行进程（父进程）的proc结构体
	for(p2 = &proc[0]; p2 < &proc[NPROC]; p2++) // 从起始位置遍历proc[],寻找未使用的元素，找到后将p2指向该元素，然后跳转到found
		if(p2->p_stat == NULL)
			goto found;
	u.u_error = EAGAIN;
	goto out;

found: // 找到未使用的元素后，调用newproc()生成新的进程，由于newproc()向父进程返回0，因此if条件在父进程上为假
	if(newproc()) { // newproc()对子进程返回1，if判断为真
		u.u_ar0[R0] = p1->p_pid;
		u.u_cstime[0] = 0;
		u.u_cstime[1] = 0;
		u.u_stime = 0;
	...
	u.u_ar0[R0] = p2->p_pid; // 将父进程的r0设为子进程的proc.p_pid，作为系统调用fork对父进程的返回值

out:
	u.u_ar0[R7] =+ 2; // 将父进程的程序计数器指向下一条指令
}
